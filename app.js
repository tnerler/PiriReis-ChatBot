...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (chatbox) {
    const bgUrl = chatbox.getAttribute("data-back");
    if (bgUrl) {
...
let mesajGosterildi = false; // Sayfa her yüklendiğinde sıfırlanır
...
function toggleChat() {
  ...
  if (chatbox.style.display === "block" || chatbox.style.display === "flex") {
    ...
    // Sadece ilk kez açıldığında mesajı göster
    if (!mesajGosterildi) {
  ...
  function sendMessage() {
    ...
    fetch("https://chatbot.pirireis.edu.tr/ask", {
      method: "POST",
      headers: {
  ...
  function sendFeedback(feedbackId, feedbackType) {
    fetch("https://chatbot.pirireis.edu.tr/feedback", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
